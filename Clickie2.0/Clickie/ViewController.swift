...
class ViewController: UIViewController, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    var startBtnEnable = false
    var clickBtnEnable = false
    ...
    func countDownTimer() {
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
            if self.time > 0 {
                ...
                self.startBtnEnable = true
                self.clickBtnEnable = false
            }//else
        }//dispatch
    }//countDownTimer
    ...
    @IBAction func startButtonAction(_ sender: Any) {
        if startBtnEnable == true && isConnected == true {
            self.startBtnEnable = false
            self.clickBtnEnable = true
    ...
    @IBAction func clickButtonAction(_ sender: Any) {
        if startBtnEnable == false && isConnected == true {
            clicks = clicks + 1
        }//if
    }
    ...
    @IBAction func clicker(_ sender: UIButton) {
        if isConnected == true && clickBtnEnable == true {
            countDownTimer()
        }//if
    }
